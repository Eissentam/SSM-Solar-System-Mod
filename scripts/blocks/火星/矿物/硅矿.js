...
try{
    ...
    var candidates = ["太阳系模组-硅矿","硅矿","ssm-硅矿"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.硅矿 = existing; print("硅矿: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.硅矿 = null; print("硅矿: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("硅矿 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("硅矿加载失败: "+err); }