...
try{
    ...
    var candidates = ["太阳系模组-石墨矿","石墨矿","ssm-石墨矿"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.石墨矿 = existing; print("石墨矿: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.石墨矿 = null; print("石墨矿: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("石墨矿 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("石墨矿加载失败: "+err); }