...
try{
    ...
    var candidates = ["太阳系模组-土钢墙","土钢墙","ssm-土钢墙"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.土钢墙 = existing; print("土钢墙: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.土钢墙 = null; print("土钢墙: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("土钢墙 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("土钢墙加载失败: "+err); }