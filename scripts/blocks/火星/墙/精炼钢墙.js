...
try{
    ...
    var candidates = ["太阳系模组-精炼钢墙","精炼钢墙","ssm-精炼钢墙"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.精炼钢墙 = existing; print("精炼钢墙: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.精炼钢墙 = null; print("精炼钢墙: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("精炼钢墙 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("精炼钢墙加载失败: "+err); }