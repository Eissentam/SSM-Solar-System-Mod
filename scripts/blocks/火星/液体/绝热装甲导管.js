...
try{
    ...
    var candidates = ["太阳系模组-绝热装甲导管","绝热装甲导管","ssm-绝热装甲导管"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.绝热装甲导管 = existing; print("绝热装甲导管: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.绝热装甲导管 = null; print("绝热装甲导管: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("绝热装甲导管 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("绝热装甲导管加载失败: "+err); }