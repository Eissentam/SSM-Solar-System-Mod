...
try{
    ...
    var candidates = ["太阳系模组-微波发电机","微波发电机","ssm-微波发电机"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.微波发电机 = existing; print("微波发电机: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.微波发电机 = null; print("微波发电机: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("微波发电机 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("微波发电机加载失败: "+err); }