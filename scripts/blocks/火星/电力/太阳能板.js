...
try{
    ...
    var candidates = ["太阳系模组-太阳能板","太阳能板","ssm-太阳能板"];
    function init(){ try{ var existing = findBlockByNames(candidates); if(existing){ exports.太阳能板 = existing; print("太阳能板: 使用已注册的方块对象："+(existing.name||existing.localizedName)); } else { exports.太阳能板 = null; print("太阳能板: 未在内容中找到已注册方块，导出为 null（避免重复注册）"); } }catch(e){ print("太阳能板 init 异常: "+e); } }
    if(typeof ClientLoadEvent !== 'undefined' && typeof Events !== 'undefined'){ Events.on(ClientLoadEvent, init); } else init();
}catch(err){ print("太阳能板加载失败: "+err); }